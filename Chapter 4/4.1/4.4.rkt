...
(put 'lambda (lambda (exp env) (make-procedure (lambda-parameters exp)
                                                     (lambda-body exp)
                                                     env)))
(put 'and (lambda (exp env) (eval-and (operands exp) env)))
(put 'or (lambda (exp env) (eval-or (operands exp) env)))
...
(define (eval-or operands env)
  (cond ((no-operands? operands) false)
        ...
        ((true? (eval (first-operand operands) env))
         true)
        (else (eval-or (rest-operands) env))))

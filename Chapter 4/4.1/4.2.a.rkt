...
(define (eval exp env)
  (cond ((self-evaluating? exp) exp)
        ...
        ; Louis Reasoner wants to put the application? clause
        ; before assignment and definitions clause
        ; 4.2 a ANSWER
        ; how will this program react to (eval (define x 3) glob-env) ?
        ; application? is true if exp is a pair, and (define x 3) would be
        ; a pair, so (define x 3) would get treated as an application instead
        ; of a define like it should be
        ;
        ; ex (meow 3 4)
        ((application? exp)
         (apply (eval (operator exp) env)
                (list-of-values (operands exp) env)))
        ; assignment is when you use set!
        ((assignment? exp) (eval-assignment exp env))
        ; definition is when you use define
        ((definition? exp) (eval-definition exp env))
        ((if? exp) (eval-if exp env))
        ; this is for creating procedures
        ((lambda? exp)

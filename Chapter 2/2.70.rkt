...
; put x into set according to weight
...
(define tree (generate-huffman-tree leaf-pairs))

(define message '(get a job sha na na na na na na na na get a job sha na na na na 
...
; using a fixed-length code for an eight-symbol alphabet, we'd need 3 bits per symbol.
; so that would be 108 bits. with huffman encoding, we get 84 bits.

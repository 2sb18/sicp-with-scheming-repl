...
(define (install-polynomial-package)
        ...
        (let ((t1 (first-term L1))     ; 
          ...
          (if (> (order t2) (order t1))
              ...
              (let ((new-coeff (div (coeff t1) (coeff t2)))
                ...
                (let ((rest-of-result
                       (div-terms (add-terms L2 (neg-terms (mul-term-by-all-terms (make-term new-order new-coeff) L1)))
                                  L2)
                       ))
  ...
  (define (clean-terms term-list)
    (if (empty-termlist? term-list)
        '()
       (adjoin-term (first-term term-list) (rest-terms term-list))))
  ...
  (define (add-poly p1 p2)
    (oper-poly add-terms p1 p2))
  (define (neg-poly p)
    (make-poly (variable p) (neg-terms (term-list p))))
  (define (sub-poly p1 p2)
    (add-poly p1 (neg-poly p2)))
  (define (mul-poly p1 p2)
    (oper-poly mul-terms p1 p2))
  (define (oper-poly terms-operator p1 p2)
    (let ((result (terms-operator (term-list p1) (term-list p2)))
          (var (cond ((same-variable? (variable p1) (variable p2)) (variable p1))
                     ...
                     (else (error "Polynomials not in same var -- OPER-POLY" (list terms-operator p1 p2))))))
...
; adding together polynomials that should result in zero
...
(newline)
(define p94 (make-polynomial 'x '((1 1) (0 -1))))
(define p92 (make-polynomial 'x '((0 -1))))
(div p94 p92)
